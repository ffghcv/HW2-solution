...
def check_for_and_del_io_files():
    ...
    import os
    #print(os.listdir())
    A = list(['currency_pair.txt', 'currency_pair_history.csv', 'trade_order.p', 'backtest.npy',
              'backtestresult.npy', 'price.npy', 'getbands.npy', 'calendar_ledger.csc', 'blotter.csv'])
    for i in A:
        if i in os.listdir():
            os.remove(i)
